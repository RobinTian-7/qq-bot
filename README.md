...
`group_ids` 填几个都行。多群时**务必同时填 `names`**，否则日报里只能看到「群123456789」，
而且模型也分不清哪条通知属于哪个班：

```toml
[group]
group_ids = [123456789, 987654321]
names = { "123456789" = "三年二班", "987654321" = "英语提高班" }
```

配好之后：

- 消息进模型前会带上 `【三年二班】` 前缀，提示词里明确要求**不同群的事不合并成一条**
- 日报里每个条目带群名标签；只监控一个群时自动不显示（标了是噪音）
- 想单独出某个群：`qq-agent report -g 英语提高班`，文件名会带群名后缀，不会覆盖合并版

一个进程监听所有群，不需要为每个群跑一份。

**一处需要注意的差别**：`teacher_qqs` 和 `teacher_name_keywords` 是**全局**的，
某个 QQ 号在 A 群算老师，在 B 群也会被算作老师。
`include_admins` 则是**按群独立**的——各群的成员列表分别拉取，群主/管理员身份不会串。
如果你的群之间老师完全不重叠，靠 `include_admins` 就够了。
